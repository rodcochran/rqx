...
def _query(url) -> str:
    """`resp.url` is an `rqx.URL` (https://github.com/rodcochran/rqx/issues/59),
    so go through `str()` rather than slicing it directly."""
    url = str(url)
...
def test_none_value_sends_an_empty_value(flaky_server):
    """httpx's `primitive_value_to_str(None)` is `""`, so the key stays with
    nothing after the `=` (https://github.com/rodcochran/rqx/issues/59)."""
    resp = rqx.get(f"{flaky_server}/streamable", params={"keep": "x", "blank": None})
    assert _query(resp.url) == "keep=x&blank="


def test_only_none_values_still_send_the_keys(flaky_server):
    resp = rqx.get(f"{flaky_server}/streamable", params={"blank": None})
    assert _query(resp.url) == "blank="
...
def test_a_query_string_is_accepted_as_params(flaky_server):
    resp = rqx.get(f"{flaky_server}/streamable", params="page=1&page=2")
    assert _query(resp.url) == "page=1&page=2"


def test_an_unusable_params_type_raises_type_error(flaky_server):
    with pytest.raises(TypeError, match="mapping"):
        rqx.get(f"{flaky_server}/streamable", params=1)  # ty: ignore[invalid-argument-type]


def test_sequence_values_repeat_the_key(flaky_server):
    resp = rqx.get(f"{flaky_server}/streamable", params={"ids": [1, 2]})
    assert _query(resp.url) == "ids=1&ids=2"
...
def test_unsupported_value_type_raises_type_error(flaky_server):
    with pytest.raises(TypeError, match="str, int, float, bool, or None"):
        rqx.get(f"{flaky_server}/streamable", params={"k": {"nested": 1}})  # ty: ignore[invalid-argument-type]
...
async def test_async_client_shares_the_coercion(flaky_server):
    ...
    assert _query(resp.url) == "page=1&active=true&ratio=0.5&empty="
...
def test_params_replace_a_query_already_on_the_url(flaky_server):
    """httpx's rule: `params=` is the query, not an addition to it
