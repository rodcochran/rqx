...
def _wire(client, flaky_server, payload):
    return client.post(f"{flaky_server}/echo-body", json=payload).json()["body"]


@given(json_value_no_floats)
def test_encode_matches_stdlib_byte_for_byte(flaky_server, client, payload):
    ...
    assert _wire(client, flaky_server, payload) == expected


@given(json_value)
def test_encode_is_value_equivalent_to_stdlib(flaky_server, client, payload):
    """Floats may print differently (`1e16` vs `1e+16`) but must parse equal."""
    assume(payload is not None)
    assert json.loads(_wire(client, flaky_server, payload)) == payload


@given(json_value)
def test_decode_round_trips_with_key_order(flaky_server, client, payload):
    ...
    resp = client.post(f"{flaky_server}/reflect-json", content=body)
    assert json.dumps(resp.json()) == json.dumps(payload)


@given(st.one_of(st.integers(max_value=-(2**63) - 1), st.integers(min_value=2**64)))
def test_ints_past_64_bits_always_raise_overflow_error(flaky_server, client, n):
    try:
        client.post(f"{flaky_server}/echo-body", json={"n": n})
